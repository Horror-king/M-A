...
async function saveBotResponseToSupabase(content, originalCommand, commandType = 'AI') {
  try {
    ...
    // Ensure bot is marked online in the onlineUsers map and broadcast
...
// ===== MODIFIED: COMMAND API HANDLER WITH IMPROVED REPLY IMAGE EXTRACTION =====
 app.post("/api/command", async (req, res) => {
  try {
    ...
    // ===== FIXED: Handle -prompt command globally (any source) =====
    if (message.trim().startsWith('-prompt')) {
      // Extract everything after '-prompt' and trim
      const rest = message.trim().slice(7).trim();
      let imageUrl = null;
      let userPrompt = null;

      // If there is a reply image URL from frontend, use it
      if (reply_image_url) {
        imageUrl = reply_image_url;
      }
      // Check if the first part of rest is a URL (starts with http:// or https://)
      else if (rest.startsWith('http://') || rest.startsWith('https://')) {
        // First token might be the URL (if there are multiple words, take the first as URL)
        const parts = rest.split(/\s+/);
        if (parts[0].startsWith('http://') || parts[0].startsWith('https://')) {
          imageUrl = parts[0];
          // The rest after URL could be ignored or used as extra context - for simplicity, ignore extra text
          if (parts.length > 1) {
        ...
        // No URL found, treat as text prompt
        userPrompt = rest;
      }

      // Validate we have either imageUrl or userPrompt
      ...
      try {
        ...
        // Return the specific error message from handlePromptCommand
