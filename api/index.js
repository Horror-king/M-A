...
if (!process.env.VERCEL) {
  ...
  console.log('⚠️ Socket.io disabled on Vercel');
  io = { 
    on: () => {}, 
    emit: () => {},
    to: () => ({ emit: () => {} })
...
if (process.env.VERCEL) {
  app.use((req, res, next) => {
    console.log(`🌐 Vercel Request: ${req.method} ${req.path}`);
    next();
  });
}
  ...
  try {
    ...
    for (const testCmd of testCommands) {
      try {
        ...
        const response = await axios.post(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/command`, testCmd, {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('send-private-message', async (data) => {
      try {
        ...
        const response = await axios.post(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/ai/private`, {
...
if (process.env.VERCEL) {
  ...
  module.exports = app;
  console.log('🚀 Vercel serverless mode - App exported as serverless function');
} else {
  // Start server for traditional hosting
  server.listen(port, () => {
    console.log(`🚀 Server running on port ${port}`);
    console.log(`🔹 Command prefix: "${PREFIX}"`);
    console.log(`👥 Online users tracking: ${process.env.VERCEL ? 'DISABLED on Vercel' : 'ACTIVE (3 minute timeout)'}`);
    ...
    console.log(`💬 Real-time messaging: ${process.env.VERCEL ? 'DISABLED on Vercel' : 'ENABLED via Socket.io'}`);
    console.log(`🔌 Socket.io events: ${process.env.VERCEL ? 'DISABLED' : 'new-message, message-deleted, user-status-change'}`);
